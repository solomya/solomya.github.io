...
// Перетаскивание блоков. На вход подаём количество допустимых дочерних элементов (включая изначально существующие доч.эл., если они есть). При попытке присоединить лишний элемент, последний элемент удаляется из родителя и переносится на общее поле. Имеет смысл использовать, если присоединить к родитель можно ТОЛЬКО 1 дочерний элемент. Модильное окно не выводится
